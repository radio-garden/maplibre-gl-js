// Shaders are prepared lazily into the `registry.shader` map by the
// register* feature functions (see src/features.ts), so that unused shaders
// tree-shake out. This module only provides the `prepare`/transpile helpers
// and the PreparedShader type.
