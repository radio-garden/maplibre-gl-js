...
export class WorkerTile {
    ...
    async parse(data: VectorTileLike, layerIndex: StyleLayerIndex, availableImages: string[], actor: IActor, subdivisionGranularity: SubdivisionGranularitySetting): Promise<WorkerTileResult> {
        ...
        for (const key in buckets) {
            ...
            if (registry.bucket.symbol && bucket instanceof registry.bucket.symbol) {
                ...
                (registry.bucket.line && bucket instanceof registry.bucket.line) ||
                (registry.bucket.fill && bucket instanceof registry.bucket.fill) ||
                (registry.bucket['fill-extrusion'] && bucket instanceof registry.bucket['fill-extrusion']))) {
                recalculateLayers(bucket.layers, this.zoom, availableImages);
